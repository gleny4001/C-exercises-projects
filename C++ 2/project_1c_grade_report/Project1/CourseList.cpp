...
// addCourse
void CourseList::addCourse(const Course& list)
{
...
// getCourseUnits
double CourseList::getCourseUnits(const string& prefix, int courseNum) const
{
	for (int i = 0; i <= totalCourses; ++i)
	{
...
{
	int iter = 0;
	while (iter < totalCourses)
	{
		if (courseList[iter].getCoursePrefix() == prefix
			&& courseList[iter].getCourseNo() == courseNum)
			return true;
		iter++;
	}
	return false;
